...
| `note_embeddings` | `generate-note-embedding` (INSERT/UPDATE via service role), `suggest-note-connections` (busca por similaridade), `search_notes` (semântica opcional) | — (uso interno, sem página direta) | Vetor `pgvector(768)` (Gemini). Escrito só por Edge Functions de IA; SELECT restrito ao dono. |
...
| `generate-note-embedding` (Edge) | `notes` (leitura), `note_embeddings` (INSERT/UPDATE) | — (trigger interno após criar/editar nota; backfill) | Gera embedding via Gemini `gemini-embedding-001` → `vector(768)`. Service role. |
